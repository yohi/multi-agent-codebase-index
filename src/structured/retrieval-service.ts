...
export class SymbolRetrievalService {
  ...
  async getSymbolSource(input: { symbolId: string; signal?: AbortSignal }): Promise<SourceStatus & { request?: { symbolId: string } }> {
    ...
    const domain = this.checkGlobalState(state);
    if (domain !== undefined) {
      return { ...domain, request: { symbolId: input.symbolId } };
    }

    const resolution = await this.resolveSymbol(input.symbolId);
    if (resolution.kind === 'tombstone') {
      return { status: 'not_found', reasonCode: 'SYMBOL_RETIRED', request: { symbolId: input.symbolId } };
    }
    if (resolution.kind === 'missing') {
      return { status: 'not_found', reasonCode: 'FILE_NOT_FOUND', request: { symbolId: input.symbolId } };
    }
    if (resolution.kind === 'pending') {
      return { status: 'index_incomplete', reasonCode: 'INDEX_PENDING_GENERATION', request: { symbolId: input.symbolId } };
    }
    ...
    if (readResult.status !== 'ok') {
      return { status: 'stale', reasonCode: readResult.reasonCode, request: { symbolId: input.symbolId } };
    }
    ...
    if (sliceHash !== resolution.declaration.sourceHash) {
      return { status: 'stale', reasonCode: 'INDEX_FILE_HASH_MISMATCH', request: { symbolId: input.symbolId } };
    }
  ...
  async getSymbolContext(input: { symbolId: string; tokenBudget: number; signal?: AbortSignal }): Promise<unknown> {
    const sourceResult = await this.getSymbolSource({ symbolId: input.symbolId, signal: input.signal });
    if (sourceResult.status !== 'ok') {
      return { ...sourceResult, request: { symbolId: input.symbolId, tokenBudget: input.tokenBudget } };
    }
    return { status: 'ok', freshness: 'fresh', context: sourceResult.source, request: { symbolId: input.symbolId, tokenBudget: input.tokenBudget } };
  }
