...
| `watcher.ignorePaths`       | string list      | `['node_modules', '.agents/', 'AGENTS.md', '.git', '.claude', '.worktrees', '.nexus', '.nexus/', 'dist', 'build', 'out', 'coverage', '.cache', '.parcel-cache', 'venv', '.venv', 'env', '.idea', '.vscode', '.DS_Store', 'package-lock.json', 'pnpm-lock.yaml', 'yarn.lock', 'bun.lockb', '*.lock', '__pycache__', '*.pyc', '.pytest_cache', '.mypy_cache', '.ruff_cache']` | `NEXUS_WATCHER_IGNORE_PATHS`        | 監視・インデックス対象外とするパスのリスト              |
